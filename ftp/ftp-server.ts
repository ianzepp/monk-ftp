/**
 * Monk Minimal FTP Server - Start Small and Verify Each Step
 */
...
export class MinimalFtpServer {
    ...
    async stop(): Promise<void> {
        return new Promise((resolve) => {
            console.log(`🎯 STOPPING FTP server on port ${this.port}`);
    ...
    private handleConnection(socket: net.Socket): void {
        ...
        socket.on('close', () => this.cleanupConnection(connection));
        socket.on('error', (e) => {
    ...
    private cleanupConnection(conn: Connection): void {
        console.log(`🎯 [${conn.id}] CLEANING UP CONNECTION`);
        
        // Close data server if active
        if (conn.dataServer) {
            console.log(`🎯 [${conn.id}] CLOSING DATA SERVER`);
            conn.dataServer.close();
            conn.dataServer = undefined;
        }
        
        // Close data socket if active
        if (conn.dataSocket) {
            console.log(`🎯 [${conn.id}] CLOSING DATA SOCKET`);
        ...
        console.log(`🎯 [${conn.id}] CONNECTION CLEANUP COMPLETE`);
    }
